...
def main():
    data_dir = '../datasets/VisDrone2019-CROPS-224-AUGMENTATION'
    batch_size = 32
    num_epochs = 20
    lr = 0.0001
    patience = 5            
    ...
    nome_peso = f"pesos/ffc_resnet50_fc_only_bs{batch_size}_ep{num_epochs}_lr{str(lr).replace('.', '')}_bal_{datetime.now().strftime('%Y%m%d-%H%M%S')}.pth"
    ...
    # Modelo FFC ResNet50, carregar pesos do ImageNet
    model = ffc_resnet50(pretrained=False)
    checkpoint_path = 'pesos/FFC_ResNet_50.pth'
    checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=False)

    # Remove pesos da última camada para adaptar ao seu número de classes
    ...
    # Congelar todos os parâmetros, exceto a última camada fc
